...
def mock_motor_client():
    ...
    with patch("motor.motor_asyncio.AsyncIOMotorClient") as mock_client:
        ...
        mock_db.__getitem__.side_effect = lambda x: (
            mock_experiments if x == "experiments" else mock_data
        )
...
async def test_find_experiment_single_match(mock_motor_client):
    ...
    mock_cursor.to_list = AsyncMock(
        return_value=[{"experimentId": "exp1", "Date": "2023-05-01"}]
    )
...
async def test_import_experiment_sheet(mock_motor_client, sample_experiment_df):
    ...
    with patch("pandas.read_excel", return_value=sample_experiment_df):
        with patch.object(service, "cleanData", return_value=sample_experiment_df):
            with patch.object(
                service, "isExpDuplicate", new_callable=AsyncMock
...
async def test_import_data_sheet_with_match(mock_motor_client, sample_data_df):
    ...
    with patch("pandas.read_excel", return_value=sample_data_df):
        with patch.object(service, "cleanData", return_value=sample_data_df):
            with patch.object(
                service, "findExperiment", new_callable=AsyncMock
            ) as mock_find_exp:
                ...
                with patch.object(
                    service, "linkData", new_callable=AsyncMock
                ) as mock_link_data:
                    sample_records = [
                        {
                            "dataSheetId": "data1",
                            "experimentId": "matched_experiment_id",
                        },
                        {
                            "dataSheetId": "data2",
                            "experimentId": "matched_experiment_id",
...
async def test_migrate(mock_motor_client):
    ...
    with patch.object(
        service, "importExperimentSheet", new_callable=AsyncMock
    ) as mock_import_exp:
        with patch.object(
            service, "importDataSheet", new_callable=AsyncMock
        ) as mock_import_data:
