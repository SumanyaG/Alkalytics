...
\begin{landscape}
    \begin{longtable}{|p{2.5cm}|p{4cm}|p{4cm}|p{4cm}|p{4cm}|p{1.25cm}|p{1cm}|}
        ...
        and rejection logic & SR-5, FR-4, SR-10 & H2-3 \\
        \cline{1-7}
        ...
        timeout settings & FR-14, SR-11, SR-4 & H5-3 \\
        \cline{1-7}
        ...
        uncompleted & JavaScript errors, resource overload & Debug UI code,
        efficient resource management & FR-7, PR-3 & H7-1 \\
        \cline{2-7}
...
\textbf{SR-2.} Only authenticated users should have the ability to query or
  ...
  \ \\
\textbf{SR-3.} The system must restrict sensitive operations (e.g., data export)
...
prevent corrupted or incomplete data entries.
  ...
  \end{itemize}
\ \\
\textbf{SR-10.} Duplicate data entries must be automatically detected and
rejected to maintain data integrity.
  \begin{itemize}
    \item \textit{Rationale:} Prevents inconsistencies and conflicting results
    caused by duplicate entries.
    \item \textit{Fit Criterion:} The system shall automatically detect and
    block duplicate data entries.
    \item \textit{Traceability:} SR-7, FR-4.
  \end{itemize}
\ \\
\textbf{SR-11.} The application must detect and mitigate suspicious activities
such as repeated failed logins, triggering a security alert.
  \begin{itemize}
    \item \textit{Rationale:} Enhances system security by detecting potential
    unauthorized access attempts.
    \item \textit{Fit Criterion:} The system must detect and block unauthorized
    login attempts after three failures.
    \item \textit{Traceability:} SR-15.
  \end{itemize}

\subsection{Privacy Requirements}
\textbf{SR-12.} All personal information related to experimental participants or
  stakeholders, if applicable, must be anonymized and handled in accordance with
  relevant privacy laws and regulations. \ \\
\ \\
\textbf{SR-13.} The application must restrict data sharing with external parties
  unless expressly authorized by stakeholders, and users must be fully informed
  about the privacy policies. \ \\
  \ \\
  \textbf{SR-14.} The system must monitor database storage capacity and alert
  administrators when thresholds are reached to prevent system crashes.
    \begin{itemize}
      \item \textit{Rationale:} Ensures the system continues operating smoothly
...
\textbf{SR-15.} The application must maintain a comprehensive audit trail,
  logging all access and modification events, including timestamps and
  identities of users performing actions. \ \\
\ \\
\textbf{SR-16.} Audit logs must be securely stored and accessible only by
  authorized personnel. \ \\
  \ \\
  \textbf{SR-17.} The system must provide real-time error logging and display
  error messages to users to enhance troubleshooting.
    \begin{itemize}
      \item \textit{Rationale:} Ensures users are informed about system issues
...
\textbf{SR-18.} The application must have proactive measures to detect and
  ...
  \ \\
  \textbf{SR-19.} Real-time monitoring and optimization of system resources must
  be implemented to avoid crashes due to resource overload.
    \begin{itemize}
      \item \textit{Rationale:} Prevents system downtime by ensuring efficient
...
timeline? Which requirements will be implemented in the future?}
