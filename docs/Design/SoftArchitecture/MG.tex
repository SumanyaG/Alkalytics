...
=======
  ...
  \item[Services:] This module handles the storage of system data by interacting with
  a NoSQL database. It transforms the data received as CSV files into a format suitable
  for storage in the database using a custom algorithm. This module also provides
  interfaces to query, retrieve, and update stored data.
  \item[Implemented By:] Software Engineering
  \item[Rationale:] This module abstracts the complexities of the NoSQL database
...
\begin{description}
  ...
  \item 
  \lstinline{DB_CONNECTION_STRING}: The connection string for accessing the
    NoSQL database.
  \item
  \lstinline{MAX_BATCH_SIZE}: The maximum number of rows to process and upload
  in a single batch.
\end{description}
...
\begin{description}
  ...
  \begin{table}[h!]
    ...
    \lstinline{initializeDB} & - & Boolean & Throws \lstinline{DatabaseInitializationError}
    if the database connection fails.\\
    \midrule
    
    \lstinline{uploadData} & filePath(string) & Boolean & Throws \lstinline{FileNotFoundError}
    if the file does not exist, or \lstinline{DataTransformationError} if the Transformation
    fails.\\
    \midrule

    \lstinline{queryData} & query(string) & Data (JSON) & Throws \lstinline{QueryExecutionError}
    if the query is invalid.\\
    \midrule

    \lstinline{updateData} & query(string), updates & Boolean & Throws \lstinline{UpdateError}
    if the update fails.\\
    \bottomrule
...
\begin{description}
  ...
  \item
  \lstinline{database}: Represents the connection to the NoSQL database.
  \item
  \lstinline{transformationRules}: Defines the set of rules for transforming CSV data to
  JSON format.
  ...
  \item
  \begin{itemize}
    \item \textbf{Output}: Returns \lstinline{True} if the database is successfully initialized.
    Throws a\\ \lstinline{DatabaseInitializationError} otherwise.
  \end{itemize}
  \item 

  \lstinline{uploadData(filePath: string)}:
  \item
  \begin{itemize}
    \item \textbf{Transition}: Reads the specified CSV file, transforms the contents into
    JSON format using \lstinline{transformationRules}, and uploads the transformed data to
    the database in batches.
  \end{itemize}
  \item
  \begin{itemize}
    \item \textbf{Output}: Returns \lstinline{True} if successful. Throws \lstinline{FileNotFoundError}
    if the file does not exist or \lstinline{DataTransformationError} if the transformation fails.
  \end{itemize}
  \item

  \lstinline{queryData(query: string)}:
  \item
  \begin{itemize}
  ...
  \item 
  \begin{itemize}
    \item \textbf{Output}: Returns the retrieved data in JSON format. Throws \lstinline{QueryExecutionError}
    if the query is invalid or fails.
  \end{itemize}
  \item

  \lstinline{updateData(query: string, updates)}:
  \item
  \begin{itemize}
  ...
  \item
  \begin{itemize}
    \item \textbf{Output}: Returns \lstinline{True} if successful. Throws \lstinline{UpdateError}
  ...
  \item[Local Function:] 
  \item
  \lstinline{transformCSVToJSON(filePath: string)}:
  \item
  \begin{itemize}
    \item \textbf{Description}: Reads a CSV file and converts its rows into JSON objects based on
    \lstinline{transformationRules}.
  \end{itemize}
  \item 
  \begin{itemize}
    \item \textbf{Output}: Returns a list of JSON objects or throws \lstinline{DataTransformationError}
    if the transformation fails.
  \end{itemize}
  \item 
  \lstinline{validateConnection()}:
  \item 
  \begin{itemize}
    \item \textbf{Description}: Checks the connection to the NoSQL database and returns \lstinline{True}
    if valid.
  \end{itemize}
\end{description}
...
\begin{description}
  ...
  \begin{table}[H]
    \centering
    \begin{tabular}{p{0.45\textwidth} p{0.13\textwidth} p{0.13\textwidth} p{0.2\textwidth}}
    \toprule
