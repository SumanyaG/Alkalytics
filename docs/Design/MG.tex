...
\item
  \begin{itemize}
    \item \texttt{validateVoltage(v: Real)}:
      \begin{itemize}
        \item output := \texttt{MIN\_VOLTAGE} $\leq v \leq$ \texttt{MAX\_VOLTAGE}
        \item if $\lnot$ output: \texttt{error\_flags} $\mathrel{+}= \texttt{Voltage\_Error}$
      \end{itemize}
  \end{itemize}
  \item
  \begin{itemize}
    \item \texttt{validateCurrent(i: Real)}:
      \begin{itemize}
        \item output := \texttt{MIN\_CURRENT} $\leq i \leq$ \texttt{MAX\_CURRENT}
        \item if $\lnot$ output: \texttt{error\_flags} $\mathrel{+}= \texttt{Current\_Error}$
      \end{itemize} 
  \end{itemize}
  \item
  \begin{itemize}
    \item \texttt{validateFlowRate(f: Real)}:
      \begin{itemize}
        \item output := \texttt{FLOW\_RATE\_THRESHOLD}[0] $\leq f \leq$ \texttt{FLOW\_RATE\_THRESHOLD}[1]
        \item if $\lnot$ output: \texttt{error\_flags} $\mathrel{+}= \texttt{Flow\_Rate\_Error}$
      \end{itemize}
  \end{itemize}
  \item
  \begin{itemize}
    \item \texttt{validatePH(p: Real)}:
      \begin{itemize}
        \item output := \texttt{PH\_RANGE}[0] $\leq p \leq$ \texttt{PH\_RANGE}[1]
        \item if $\lnot$ output: \texttt{error\_flags} $\mathrel{+}= \texttt{PH\_Error}$
      \end{itemize}
  \end{itemize}
  \item
  \begin{itemize}
...
\begin{enumerate}
  ...
  \newline
  Given unlimited resources, especially, more time and manpower, our team can implement the orange leaf modules 
  shown in figure \ref{fig:FigMH}. As described earlier in this document, these modules are necessary for the system 
  but will not be implemented for Rev 0 due to timing constraints. These modules have been identified as important 
  functionalities that would be great inclusions to the existing Alkalytics system. In addition to this, we would also 
  implement our stretch goals identified in the \href{https://github.com/SumanyaG/Alkalytics/blob/main/docs/ProblemStatementAndGoals/ProblemStatement.pdf}{Problem Statement and Goals}
  document last semester.
