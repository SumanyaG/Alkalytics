...
Alkalytics environment. We recommend using an Integrated Development Environment (IDE)
such as \href{https://code.visualstudio.com/}{VS Code} for ease of handling the application.
...
\end{verbatim}
    ...
    Tip: To switch between directories use the command \verb|cd <directory_name>| to 
    naviagte to a specific directory or \verb|cd ..| to go back one level.
\begin{enumerate}
        \item With \verb|src/backend| as your current directory, start backend:
\begin{verbatim}
uvicorn api:app --reload --host 127.0.0.1 --port 8000
\end{verbatim}
        \item With \verb|src/frontend| as your current directory, start server:
\begin{verbatim}
yarn ts-node src/utils/server.ts
\end{verbatim}
        \item With \verb|src/frontend| as your current directory, start frontend:
\begin{verbatim}
yarn start
\end{verbatim}
    \end{enumerate}
